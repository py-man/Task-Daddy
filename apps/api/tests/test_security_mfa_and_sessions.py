...
async def test_admin_mfa_enrollment_and_admin_guard(client: AsyncClient) -> None:
  ...
  await login(client, "admin@neonlanes.local", "admin1234", totpCode=totp_code(secret, now=int(time.time())))

  res = await client.post("/users", json={"email": "mfa.user@neonlanes.local", "name": "MFA User", "role": "member"})
...
async def test_mfa_confirm_marks_current_session_verified_for_admin_actions(client: AsyncClient) -> None:
  ...
  created = await client.post("/users", json={"email": "post.confirm@neonlanes.local", "name": "Post Confirm", "role": "member"})
...
async def test_recovery_code_is_one_time_use(client: AsyncClient) -> None:
  ...
  await login(client, "admin@neonlanes.local", "admin1234", recoveryCode=first)
  # Reusing should fail.
  res = await client.post("/auth/login", json={"email": "admin@neonlanes.local", "password": "admin1234", "recoveryCode": first})
...
async def test_trusted_device_skips_mfa_and_can_be_revoked(client: AsyncClient) -> None:
  ...
  await login(client, "admin@neonlanes.local", "admin1234", totpCode=totp_code(secret, now=int(time.time())), rememberDevice=True)
  ...
  no_mfa = await client.post("/auth/login", json={"email": "admin@neonlanes.local", "password": "admin1234"})
  ...
  client.cookies.delete("nl_session")
  must_mfa = await client.post("/auth/login", json={"email": "admin@neonlanes.local", "password": "admin1234"})
...
async def test_global_session_revoke_requires_admin_mfa_and_clears_other_sessions(client: AsyncClient) -> None:
  ...
  try:
    await login(primary, "admin@neonlanes.local", "admin1234", totpCode=totp_code(secret, now=int(time.time())))
    await login(secondary, "admin@neonlanes.local", "admin1234", totpCode=totp_code(secret, now=int(time.time())))
